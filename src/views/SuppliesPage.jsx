...
export default function SuppliesPage() {
  ...
  return (
    <div className="page-wrap">
      ...
      {loading.products ? <ListingsSkeleton /> : visibleListings.length ? <div className="listing-grid">{visibleListings.map((listing) => <ListingCard key={listing.id} listing={listing} type="product" onAdd={addToCart} onMessage={messageSeller} isOwner={listing.ownerId === user?.id} onEdit={setEditing} onDelete={deleteListing} />)}</div> : <EmptyState title="No listings match those filters" detail="Try a different search or clear one of the filters." action={<button className="button button-secondary" onClick={() => { setSearch(''); setCategory(''); setCounty(''); setKind('all'); }}>Clear filters</button>} />}
